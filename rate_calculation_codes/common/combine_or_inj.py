...
parser.add_argument('--fap-threshold',
                    help='Exclusive FAP threshold', type=float, default=0.2513)
...
df_cwb = pd.read_csv(cwb_bin_file, usecols=['Inj Time', 'cWB FAP', 'massbin'])
df_cwb = df_cwb.round({'Inj Time': 0})

print('MassBin Number ',df_cwb['massbin'][0])
# Reading two columns of the CSV Files and rounding Inj Time to nearest sec
df_pycbc = pd.read_csv(pycbc_bin_file, usecols=['Inj Time', 'PyCBC FAP'])
df_pycbc = df_pycbc.round({'Inj Time': 0})


# Reading two columns of the CSV Files and rounding Inj Time to nearest sec
df_gstlal = pd.read_csv(gstlal_bin_file, 'r', delimiter=',')
